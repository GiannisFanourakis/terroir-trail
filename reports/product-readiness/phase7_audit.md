...
7. **Backend pass verification privacy and authorization review required.** Public App no longer opens `HostVerificationModal` from `?verify_pass=...` URLs. The existing `GET /api/passes/verify/:passId` backend/private-pilot verification flow still requires a separate privacy and authorization audit before Explorer Pass can ever be considered for public activation.
...
3. Conduct separate privacy and authorization review for backend/private-pilot Explorer Pass verification infrastructure before any future commercial activation.
