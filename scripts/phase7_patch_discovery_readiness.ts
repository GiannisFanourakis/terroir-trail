...
try {
  // App: surface catalogue loading/fallback state and lazy-modal loading feedback.
  {
  ...
  // Map: never navigate to unresolved coordinates, fix Peskesi's legacy-category label,
  // avoid empty rating UI, and improve control accessibility.
  {
    const state = load('src/components/Map/MapCanvas.tsx');
    replaceOnce(
      state,
      'Peskesi map marker override start',
    ...
    replaceOnce(
      state,
      'Peskesi map marker override close',
      `      case 'apiary':\n        icon = '🍯';\n        iconBg = 'bg-orange-500/25 text-orange-200 border-orange-500/50';\n        break;\n    }\n\n    const shortVillage`,
      `      case 'apiary':\n        icon = '🍯';\n        iconBg = 'bg-orange-500/25 text-orange-200 border-orange-500/50';\n        break;\n      }\n    }\n\n    const shortVillage`
    );
    replaceOnce(
      state,
      'unresolved map fly-to guard',
    ...
    replaceOnce(
      state,
      'quick-card close aria',
      `                      className="text-stone-400 hover:text-white p-1"\n                    >\n                      <X`,
      `                      className="text-stone-400 hover:text-white p-1"\n                      aria-label="Close producer preview"\n                    >\n                      <X`
    );

    const controlLabels: Array<[string, string, string]> = [
      ['location error dismiss', 'title="Dismiss"', 'title="Dismiss"\n              aria-label="Dismiss location error"'],
    ...
    for (const [label, before, after] of controlLabels) replaceOnce(state, label, before, after);
    replaceOnce(
      state,
      'pin density aria',
  ...
  // Drawer: remove synthetic VIP defaults, fix legacy farm/category wording,
  // make visiting language neutral, and improve dialog/accessibility semantics.
  {
    ...
    replaceOnce(
      ...
      `  const getCategoryDetails = (p: Producer) => {\n    if (p.id === 'peskesi-farm-kazani') {\n      return { label: 'Organic Farm', icon: '🌿', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' };\n    }\n    switch (p.category) {\n      case 'winery':\n        return { label: 'Winery', icon: '🍇', color: 'text-rose-400 bg-rose-500/10 border-rose-500/20' };\n      case 'brewery':\n        return { label: 'Brewery', icon: '🍺', color: 'text-amber-300 bg-amber-400/15 border-amber-400/30' };\n      case 'kazani':\n        return { label: 'Rakokazano', icon: '🏺', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' };\n      case 'olive_mill':\n        return { label: 'Olive Mill', icon: '🫒', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' };\n      case 'cheese_dairy':\n        return { label: 'Dairy', icon: '🧀', color: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20' };\n      case 'apiary':\n        return { label: 'Apiary / Honey', icon: '🍯', color: 'text-orange-400 bg-orange-500/10 border-orange-500/20' };\n      default:\n        return { label: 'Producer', icon: '🌿', color: 'text-stone-300 bg-stone-500/10 border-white/10' };\n    }\n  };`
    );
    replaceOnce(state, 'category details call', '  const cat = getCategoryDetails(producer.category);', '  const cat = getCategoryDetails(producer);');
    replaceOnce(
      state,
      'neutral visiting terminology',
    ...
    replaceOnce(state, 'producer highlights heading', '                Signature Tastings & Pours', '                Producer Highlights');
    replaceOnce(state, 'visitor notice label', '                Live Estate Bulletin', '                Visitor Notice');
    replaceOnce(state, 'media verification wording', '                    <span>Verified Archival Media</span>', '                    <span>Source-listed media</span>');
    replaceOnce(state, 'media rights wording', '                      Clear Rights', '                      Source listed');
    replaceOnce(
      state,
      'producer login prompt',
  ...
  // Data provenance: a failed live request must explicitly return the hook to fallback mode.
  {
    const state = load('src/services/producerService.ts');
    replaceOnce(
      state,
      'producer fetch fallback provenance',
  ...
  // Canonical roadmap: Phase 7 is already active; do not mark individual audits complete until final verification/deploy.
  {
