-- First-class terroir region storage, seeded with Thessaly from the official
-- Eurostat/GISCO NUTS 2024 NUTS-2 boundary (EL61).
--
...
WITH response AS (
  ...
  WHERE f.value -> 'properties' ->> 'NUTS_ID' = 'EL61'
)
...
SELECT
  ...
  g.geom,
  'https://gisco-services.ec.europa.eu/distribution/v2/nuts/geojson/NUTS_RG_03M_2024_4326_LEVL_2.geojson',
  'Boundary: Eurostat/GISCO NUTS 2024, NUTS-2 EL61, EPSG:4326, 1:3M.'
FROM feature,
LATERAL (
  SELECT ST_Multi(
    ST_SetSRID(
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'GISCO NUTS feature EL61 was not found; Thessaly region was not imported';
  END IF;
END $$;
