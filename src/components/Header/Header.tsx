...
export const Header: React.FC<HeaderProps> = ({
  ...
  const { canInstall, isIos, isIosSafari, requestInstall } = pwaInstall;
  ...
  const visibleDestinations =
    countryScope === 'all'
      ...
      : destinations.filter(
          (destination) =>
            getDestinationCountry(destination.id) === countryScope
        );
  const activeCountry =
    COUNTRY_LAYERS.find((country) => country.id === countryScope) ||
    COUNTRY_LAYERS[0];
  const activeDestObj =
    selectedDestination === 'all'
      ? activeCountry
        ...
        {mobileSearchOpen ? (
          ...
          <div className="flex sm:hidden items-center justify-between gap-1.5 w-full py-0.5">
            ...
            <div className="relative">
              ...
              >
                {' '}
                <span className="font-bold text-amber-300 max-w-[92px] truncate">
                  {activeDestObj.label}
                </span>
              ...
              {destMenuOpen && (
                <>
                  ...
                  <div className="absolute left-0 top-full mt-1.5 z-50 bg-stone-950 border border-white/15 rounded-2xl p-1.5 shadow-2xl flex flex-col gap-1 min-w-[220px] max-h-[calc(100dvh-5.5rem)] overflow-y-auto overscroll-contain touch-pan-y mobile-scroll backdrop-blur-xl">
                    <div className="px-2 pt-1 pb-0.5 text-[9px] uppercase tracking-[0.18em] font-bold text-stone-500">
                      Country
                    </div>
                    ...
                    {visibleDestinations.length > 0 && (
                      <>
                        ...
                        <div className="px-2 pt-0.5 pb-0.5 text-[9px] uppercase tracking-[0.18em] font-bold text-stone-500">
                          Terroir regions
                        </div>
        ...
        <div className="hidden sm:flex flex-col gap-2 w-full">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              <button
                onClick={onToggleFavoritesOnly}
                className={`flex items-center gap-1 px-2 sm:px-2.5 py-1.5 text-xs font-bold rounded-xl border transition shrink-0 cursor-pointer ${favoritesOnly ? 'bg-rose-500 text-white border-rose-400 shadow-md' : 'bg-stone-900 hover:bg-stone-850 text-stone-300 border-white/10'}`}
                title="Saved spots"
              >
                <Heart
                  className={`w-3.5 h-3.5 shrink-0 ${favoritesOnly || savedCount > 0 ? 'fill-rose-400 text-rose-400' : 'text-stone-400'}`}
                />
              ...
              {onOpenAbout && (
                <button
                  onClick={onOpenAbout}
                  className="hidden lg:flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold bg-stone-900 hover:bg-stone-850 text-stone-300 hover:text-white border border-white/10 hover:border-amber-400/40 rounded-xl transition shrink-0 cursor-pointer"
                >
              ...
              {onOpenExplorerPass && (
                <button
                  ...
                  className="hidden md:flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold rounded-xl border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/15 hover:border-amber-400/60 text-amber-200 transition shrink-0 cursor-pointer"
                >
              ...
              {isHost ? (
                onOpenProducerPortal && (
                  ...
                  >
                    ...
                    <span className="truncate max-w-[130px]">
                      {user?.producerName || 'Producer Host'}
                    </span>
                ...
                <button
                  ...
                  className="hidden md:flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold rounded-xl border border-white/10 hover:border-amber-400/50 bg-stone-900 hover:bg-stone-850 text-stone-300 hover:text-amber-300 transition shrink-0 cursor-pointer"
                >
            ...
            <div className="flex items-center gap-2 min-w-0 shrink-0">
              <label className="relative min-w-[132px]">
                ...
                <select
                  value={countryScope}
                  onChange={(event) =>
                    selectCountry(event.target.value as CountryScope)
                  }
              ...
              <label className="relative min-w-[168px]">
                ...
                <select
                  ...
                  disabled={
                    countryScope === 'all' || visibleDestinations.length === 0
                  }
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
              ...
              <button
                onClick={closeMenu}
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-stone-900 border border-white/10 text-stone-400 hover:text-white cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain touch-pan-y mobile-scroll p-4 pb-[max(1rem,env(safe-area-inset-bottom))] flex flex-col gap-2">
              {onOpenAbout && (
                <button
                  ...
                  className="flex items-center gap-3 w-full px-3 py-3 text-sm font-semibold text-stone-200 hover:text-white bg-stone-900 rounded-xl border border-white/10 cursor-pointer"
                >
              ...
              {onOpenExplorerPass && (
                <button
                  ...
                  className="flex items-center gap-3 w-full px-3 py-3 text-sm font-semibold text-amber-200 bg-amber-500/10 rounded-xl border border-amber-500/25 cursor-pointer"
                >
              ...
              {isHost ? (
                onOpenProducerPortal && (
                  <button
                    ...
                    className="flex items-center gap-3 w-full px-3 py-3 text-sm font-semibold text-amber-300 bg-amber-500/15 rounded-xl border border-amber-500/30 cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="flex items-center gap-3 w-full px-3 py-3 text-sm font-semibold text-amber-400 bg-amber-500/10 rounded-xl border border-amber-500/30 cursor-pointer"
                >
                  <Building2 className="w-4 h-4 text-amber-400" />
                  Host Portal
                </button>
              ...
              {user ? (
                <div className="mt-2 p-3.5 rounded-2xl bg-stone-900/90 border border-white/10 flex flex-col gap-3">
                  ...
                  {onOpenAccountSettings && (
                    <button
                      ...
                      className="flex items-center justify-center gap-2 w-full px-3 py-2 text-xs font-semibold text-sky-200 bg-sky-500/10 rounded-xl border border-sky-500/20 cursor-pointer"
                    >
                      Account & privacy
                    </button>
                  )}
                  {onOpenMyBookings && (
                    <button
                      ...
                      className="flex items-center justify-between w-full px-3 py-2 text-xs font-semibold text-stone-200 bg-stone-950 rounded-xl border border-white/5 cursor-pointer"
                    >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full px-3 py-2 text-xs font-bold text-rose-400 bg-rose-500/10 rounded-xl border border-rose-500/20 cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 w-full mt-2 px-4 py-3 rounded-xl bg-amber-500 text-stone-950 font-bold text-sm cursor-pointer"
                >
        ...
        <>
          ...
          >
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h2
                  id="install-terroirtrail-title"
                  className="text-base font-bold text-white"
                >
                  Add TerroirTrail to your Home Screen
                </h2>
                <p className="mt-1 text-xs leading-5 text-stone-400">
                  Keep the guide one tap away while travelling. It opens like an
                  app and stays connected to the latest web version.
                </p>
              </div>
              <button
                type="button"
