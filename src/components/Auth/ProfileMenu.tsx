...
export const ProfileMenu: React.FC<ProfileMenuProps> = ({
  ...
  const progressPercent = Math.min(
    100,
    Math.round((visitedCount / (totalProducersCount || 1)) * 100)
  );
  ...
  return (
    <div ref={menuRef} className="relative z-40 shrink-0">
      ...
      >
        ...
        <ChevronDown
          className={`w-3 h-3 text-stone-400 shrink-0 transition-transform ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      {isOpen && (
        ...
        >
          <div className="pb-3 border-b border-white/10 mb-2.5">
            <div className="flex items-center gap-2.5">
              <UserAvatar
                user={user}
                size="md"
                className="ring-2 ring-amber-500/50 shadow-md"
              />
            ...
            <div className="flex items-center justify-between mt-2 text-[10px]">
              {isAdmin ? (
                <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 font-bold border border-emerald-500/30 flex items-center gap-1">
                  ...
                  {isPlatformOwner ? 'Platform Owner' : 'TerroirTrail Admin'}
                </span>
              ) : isHost ? (
                <span className="px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-300 font-bold border border-amber-500/30 flex items-center gap-1">
                  <Building2 className="w-3 h-3" />
                  Verified Host
                </span>
              ) : (
                <span className="px-2 py-0.5 rounded-full bg-stone-900 text-stone-300 font-bold border border-white/10">
                  Traveler
                </span>
              )}
          ...
          <button
            type="button"
            onClick={() => runAndClose(onOpenPassport)}
            className="w-full text-left p-2.5 rounded-xl bg-stone-900/80 hover:bg-stone-850 border border-amber-500/20 hover:border-amber-400/40 transition mb-2 group cursor-pointer"
          >
            <div className="flex items-center justify-between text-[11px] font-bold text-stone-200 group-hover:text-amber-300 mb-1.5">
              ...
              <span className="text-amber-400">
                {visitedCount} / {totalProducersCount}
              </span>
            </div>
          ...
          <div className="space-y-1">
            <button
              type="button"
              onClick={() => runAndClose(onOpenWishlist)}
            ...
            {onOpenMyTrips && (
              <button
                type="button"
                onClick={() => runAndClose(onOpenMyTrips)}
                className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-stone-300 hover:text-white hover:bg-white/5 transition cursor-pointer"
              >
            ...
            >
              ...
              Passport stamps & notes
            </button>

            {onOpenMyBookings && (
              <button
                type="button"
                onClick={() => runAndClose(onOpenMyBookings)}
            ...
            {isAdmin && onOpenAdmin && (
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  Admin Requests & Controls
                </span>
                <span className="text-[9px] px-1.5 py-0.5 rounded bg-emerald-400 text-stone-950 uppercase">
            ...
            {isAdmin && !isHost && onOpenProducerPortal && (
              <button
                ...
                className="w-full flex items-center justify-between px-2.5 py-2 rounded-xl bg-sky-500/10 border border-sky-500/25 text-sky-200 cursor-pointer font-bold"
              >
                <span className="flex items-center gap-2">
                  <Building2 className="w-4 h-4" />
                  Preview Producer Portal
                </span>
                <span className="text-[9px] uppercase">Read only</span>
              </button>
            )}
            ...
            {isHost && onOpenProducerPortal ? (
              <button
                type="button"
                onClick={() => runAndClose(onOpenProducerPortal)}
                className="w-full flex items-center gap-2 px-2.5 py-2 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-300 cursor-pointer font-bold"
              >
                <Building2 className="w-4 h-4" />
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-stone-400 hover:text-amber-300 hover:bg-white/5 transition cursor-pointer"
              >
                <Building2 className="w-3.5 h-3.5" />
                Producer / Host access
              </button>
            ) : null}
            ...
            {onOpenAccountSettings && (
              <button
                type="button"
                onClick={() => runAndClose(onOpenAccountSettings)}
                className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-stone-300 hover:text-white hover:bg-white/5 transition cursor-pointer"
              >
            ...
            {onOpenAbout && (
              <button
                ...
                className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-stone-300 hover:text-amber-300 hover:bg-white/5 transition cursor-pointer"
              >
            ...
            {onOpenLegal && (
              <button
                ...
                className="w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-stone-400 hover:text-white hover:bg-white/5 transition cursor-pointer"
              >
                <span className="flex items-center gap-2">
                  <Scale className="w-3.5 h-3.5" />
                  Privacy & Legal
                </span>
                <span className="text-[9px]">GDPR</span>
              </button>
            )}
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 transition cursor-pointer"
            >
