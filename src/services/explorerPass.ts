...
async function request<T>(
  ...
  if (authenticated) {
    ...
    if (!auth?.currentUser)
      throw new Error(
        'Please sign in with a real account to purchase or retrieve a pass.'
      );
    headers.set(
      'Authorization',
  ...
  if (!response.ok)
    throw new Error(data.error || 'Unable to verify this pass.');
  return data as T;
}
...
export const fetchExplorerPass = () =>
  request<{ pass: ExplorerPass | null }>('/me');

export const confirmExplorerPass = (sessionId: string) =>
  request<{ pass: ExplorerPass | null }>('/confirm', {
    method: 'POST',
    body: JSON.stringify({ sessionId }),
  });

export const startPassBillingPortal = () =>
...
export async function verifyExplorerPass(input: string) {
  ...
  if (value.includes('?')) {
    try {
      passId = new URL(value).searchParams.get('verify_pass') || '';
    } catch {
      throw new Error('Enter a valid pass ID or verification link.');
    }
  }
  ...
  const { pass } = await request<{ pass: ExplorerPass }>(
    `/verify/${encodeURIComponent(passId)}`
  );
  if (
    !pass ||
